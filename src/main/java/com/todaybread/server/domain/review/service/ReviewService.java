...
/**
 * 리뷰 도메인 서비스 계층입니다.
 * 리뷰 작성, 조회, 평점 집계 등의 비즈니스 로직을 처리합니다.
 */
@Service
...
public class ReviewService {
     ...
     * 리뷰를 저장하고 가게 평점을 갱신합니다. 이미지가 있으면 함께 저장합니다.
     *
    ...
    public ReviewResponse createReview(Long userId, ReviewCreateRequest request, List<MultipartFile> images) {
        ...
        store.addReviewRating(request.rating());
    ...
    public Page<StoreReviewResponse> getStoreReviews(Long storeId, ReviewSortType sort, Pageable pageable) {
        ...
        if (reviewPage.isEmpty()) {
            return reviewPage.map(review -> null);
        }
    ...
    public Page<BossReviewResponse> getBossReviews(Long userId, BossReviewSortType sort,
        ...
        if (!userIds.isEmpty()) {
            List<Object[]> purchaseCounts = orderRepository.countByStoreIdAndStatusAndUserIdIn(
                    storeId, OrderStatus.PICKED_UP, userIds);
            for (Object[] row : purchaseCounts) {
                Long reviewUserId = (Long) row[0];
                Long count = (Long) row[1];
                purchaseCountMap.put(reviewUserId, count);
            }
        }
