...
5. SQL 적용 후 `bread_image.stored_filename`에 맞는 빵별 이미지 파일을 생성합니다.

이미지 처리:

- `scripts/seed-images/seed_store_01.*` ~ `seed_store_10.*`이 있으면 실제 이미지를 `uploads/seed_store_XX.svg` 이름으로 복사합니다.
...
개발용 seed 데이터 본문입니다. 재실행 가능하도록 기존 v2 seed 데이터와 일부 구버전 `.local` seed 데이터를 먼저 정리한 뒤 다시 삽입합니다.

삽입하는 주요 데이터:

- 일반 유저 1명: `demo-user@todaybread.com / todaybread123`
- 사장님 20명: `demo-boss1@todaybread.com` ~ `demo-boss20@todaybread.com / todaybread123`
- 매장 20개 (기존 강남권 10개 + 한성대학교 인근 5개 + 서울 전역 5개)
- 매장 이미지 20개
- 영업시간
- 빵 메뉴 120개 (기존 70개 + 추가 매장별 5개씩 50개)
- 빵 이미지 레코드
- 즐겨찾기
- 장바구니 샘플
- 과거/오늘 주문 데이터 (기존 강남권 10개 매장에만 생성)
- 주문 상태에 맞는 결제 데이터
...
- `test-order.sh`의 기본 로그인 계정은 `test-data.sql` seed 계정과 같은 `demo-user@todaybread.com`입니다.
