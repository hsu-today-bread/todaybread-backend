...
- 역할 계층: `BOSS > USER`
- 인증 불필요 경로: 회원가입, 로그인, 이메일/닉네임/전화번호 중복확인, 토큰 재발급, 헬스체크, Swagger
...
| `STORE_IMAGE_004` | 400 | 허용되지 않는 파일 형식입니다. (jpeg, png, gif, webp만 가능) |
| `STORE_IMAGE_005` | 400 | 파일 크기는 5MB를 초과할 수 없습니다. |
| `STORE_IMAGE_007` | 500 | 파일 저장에 실패했습니다. |
...
| 코드 | HTTP | 메시지 |
|------|------|--------|
| `BREAD_IMAGE_001` | 400 | 허용되지 않는 파일 형식입니다. (jpeg, png, gif, webp만 가능) |
| `BREAD_IMAGE_002` | 400 | 파일 크기는 5MB를 넘을 수 없습니다. |
| `BREAD_IMAGE_003` | 500 | 파일 저장에 실패했습니다. |
